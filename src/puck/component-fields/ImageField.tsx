...
// Appends/removes the crop query params (x1/y1/x2/y2) the media serving API understands. Also
// used by media/ImagePicker.tsx's ImageDisplay to render the block's published/edited output.
export function withCropParams(url: string, crop: CropBox | null): string {
...
// Not exported previously; now shared with media/ImagePicker.tsx's block config, which pins this
// field to bindableFields.image.overridable so a collection-wide size can be set independently of
// whichever image was used to design the card template.
export const imageSizeOverrideField: CustomField<ImageSizeOverride | null> = {
  type: "custom",
